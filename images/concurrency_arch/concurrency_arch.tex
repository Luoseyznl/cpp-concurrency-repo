...
% 移除了可能导致报错的 newtxtext 字体包，使用默认字体
...
\tikzset{
    ...
    % 各层颜色样式
    cppLayer/.style={block, draw=cCppDraw, fill=cCppFill, font=\bfseries},
...
\begin{tikzpicture}[node distance=1.0cm]
    ...
    \node [cppLayer, minimum width=11cm] (l5) {
        \textbf{L3: C++ Standard Library (Abstraction)}\\
        \texttt{std::thread}, \texttt{std::mutex}\\
        \texttt{std::condition\_variable}, \texttt{std::future}
    };

    % --- 右侧注释 L3 ---
    \node [right=0.5cm of l5, noteText] (noteL3) {
        \textbf{Interface (Standard):}\\
    ...
    % --- 右侧注释 L2 ---
    % 自动对齐到上一层注释的左侧
    \node [noteText] at (noteL3.west |- posix) {
        \textbf{Implementation (Runtime):}\\
        Allocates stack \& TLS.\\
