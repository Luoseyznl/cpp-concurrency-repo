...
\begin{tikzpicture}[node distance=1.2cm and 2.5cm]
    ...
    \node[card, fill=gray!5, text=gray!60] at ($(c2.north) + (0, 0.1)$) (c1) {\tiny HEAD\textasciitilde 1};
    ...
    % =========================================
    % 3. 核心连线 (Forward: Solid)
    % =========================================

    % [Add]: Work -> Stage
    \draw [conn] (work.15) to[out=10, in=170]
        node[cmdLabel, above=1pt] {git add}
        (stage.165);

    % [Commit]: Stage -> Local
    \draw [conn] (stage.east) to[out=0, in=180]
        node[cmdLabel, fill=white] {git commit}
        (c0.west);

    % [Push]: Local -> Remote
    \draw [conn] (local.north) to[out=90, in=340]
        node[cmdLabel, pos=0.5] {git push}
        (remote.340);

    % =========================================
    % 4. 撤销/回退连线 (Backward: Dashed)
    % =========================================

    % [Restore Work]: Work -> Stage (Dashed)
    \draw [connDashed, red!60!black] (work.345) to[out=350, in=190]
        node[cmdLabel, below=1pt] {git restore}
        (stage.195);

    % [Restore Staged]: Stage -> Local (Dashed)
    \draw [connDashed] (stage.330) to[out=330, in=180]
        node[cmdLabel, fill=none, below=6pt, xshift=5pt, align=left] {
            git restore -{}-staged
        }
        (c1.west);

    % [Reset Soft]: Local Self Loop
    \path (local.south east) edge [connDashed, loop right, min distance=1cm]
        node [right, font=\scriptsize\ttfamily, color=black!70, align=left] {
            git reset -{}-soft\\
            \itshape (HEAD only)
        } (local.east);

    % [Reset Hard]: Work -> Local (DANGER)
    ...
    % [Fetch]: Remote -> Local
    \draw [conn] (remote.240) to[out=270, in=120]
        node[cmdLabel, pos=0.6, fill=white] {git fetch}
        (local.120);
